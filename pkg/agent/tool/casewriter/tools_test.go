...
func TestNew_ThreadModeExposesTheFullWriterSet(t *testing.T) {
	// There is no status-only / assign-only subset: any host granting case
	// writes grants all of them, so a thread-mode case (board status set) carries
	// the content and assignee tools alongside case__update_case_status.
	uc := &mockCaseUC{resp: &model.Case{ID: 5}}
	tools := casewriter.New(casewriter.Deps{CaseUC: uc, WorkspaceID: "ws", CaseID: 5, StatusSet: testStatusSet(t)})
	gt.Array(t, tools).Length(4)
	gt.Value(t, toolByName(t, tools, "case__update_case")).NotNil()
