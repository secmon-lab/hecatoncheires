...
func TestAFailedToolCallTellsTheModelWhyItFailed(t *testing.T) {
	...
	llm := &mock.LLMClientMock{
		NewSessionFunc: func(_ context.Context, opts ...gollem.SessionOption) (gollem.Session, error) {
			// The failure reaches the model through the conversation, which is where
			// the strategy answers the call, so it is read off the history this
			// session was seeded with rather than off the inputs. The call that
			// follows a tool phase carries no input at all.
			cfg := gollem.NewSessionConfig(opts...)
			var seeded []gollem.Message
			if h := cfg.History(); h != nil {
				seeded = h.Messages
