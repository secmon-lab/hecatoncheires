...
func (uc *ActionUseCase) CreateAction(ctx context.Context, caseID int64, title, description string, assigneeIDs []string, slackMessageTS string, status types.ActionStatus) (*model.Action, error) {
	if title == "" {
		return nil, goerr.New("action title is required")
	}

	// Verify case exists
	if _, err := uc.repo.Case().Get(ctx, caseID); err != nil {
	...
	created, err := uc.repo.Action().Create(ctx, action)
	if err != nil {
...
func (uc *ActionUseCase) UpdateAction(ctx context.Context, id int64, caseID *int64, title, description *string, assigneeIDs []string, slackMessageTS *string, status *types.ActionStatus) (*model.Action, error) {
	// Get existing action
	existing, err := uc.repo.Action().Get(ctx, id)
	if err != nil {
	...
	updated, err := uc.repo.Action().Update(ctx, action)
	if err != nil {
...
func (uc *ActionUseCase) DeleteAction(ctx context.Context, id int64) error {
	if err := uc.repo.Action().Delete(ctx, id); err != nil {
...
func (uc *ActionUseCase) GetAction(ctx context.Context, id int64) (*model.Action, error) {
	action, err := uc.repo.Action().Get(ctx, id)
	if err != nil {
		return nil, goerr.Wrap(ErrActionNotFound, "action not found", goerr.V(ActionIDKey, id))
	}
...
func (uc *ActionUseCase) ListActions(ctx context.Context) ([]*model.Action, error) {
	actions, err := uc.repo.Action().List(ctx)
	if err != nil {
...
func (uc *ActionUseCase) GetActionsByCase(ctx context.Context, caseID int64) ([]*model.Action, error) {
	actions, err := uc.repo.Action().GetByCase(ctx, caseID)
	if err != nil {
