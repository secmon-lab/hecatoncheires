...
func TestActionUseCase_ListOpenCaseActions(t *testing.T) {
	t.Run("returns actions from open cases only", func(t *testing.T) {
		...
		openCase, err := caseUC.CreateCase(ctx, testWorkspaceID, "Open Case", "open", []string{}, nil, false, false, "", "")
		gt.NoError(t, err).Required()

		closedCase, err := caseUC.CreateCase(ctx, testWorkspaceID, "Closed Case", "closed", []string{}, nil, false, false, "", "")
		gt.NoError(t, err).Required()
	...
	t.Run("returns empty list when no actions exist", func(t *testing.T) {
		...
		_, err := caseUC.CreateCase(ctx, testWorkspaceID, "Open Case", "open", []string{}, nil, false, false, "", "")
		gt.NoError(t, err).Required()
	...
	t.Run("returns actions from multiple open cases", func(t *testing.T) {
		...
		case1, err := caseUC.CreateCase(ctx, testWorkspaceID, "Case 1", "desc1", []string{}, nil, false, false, "", "")
		gt.NoError(t, err).Required()

		case2, err := caseUC.CreateCase(ctx, testWorkspaceID, "Case 2", "desc2", []string{}, nil, false, false, "", "")
		gt.NoError(t, err).Required()
