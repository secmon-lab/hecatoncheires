...
func TestAgentUseCase_HandleAgentMention(t *testing.T) {
	t.Run("responds to mention in channel with case", func(t *testing.T) {
		...
		agentUC := usecase.NewAgentUseCase(repo, registry, slackMock, nil, nil, llmClient, agentarchive.NewMemoryHistoryRepository(), agentarchive.NewMemoryTraceRepository())
	...
	t.Run("system prompt includes case info and field values", func(t *testing.T) {
		...
		agentUC := usecase.NewAgentUseCase(repo, nil, slackMock, nil, nil, llmClient, agentarchive.NewMemoryHistoryRepository(), agentarchive.NewMemoryTraceRepository())
...
func TestAgentUseCase_HandleSessionInfoRequest(t *testing.T) {
	t.Run("opens modal with session ID", func(t *testing.T) {
		...
		agentUC := usecase.NewAgentUseCase(repo, nil, mockWithCapture, nil, nil, llmClient, agentarchive.NewMemoryHistoryRepository(), agentarchive.NewMemoryTraceRepository())
...
func TestLifecycle_AgentSession(t *testing.T) {
	...
	uc := usecase.NewAgentUseCase(repo, registry, slackMock, nil, nil, llm, historyRepo, traceRepo)
