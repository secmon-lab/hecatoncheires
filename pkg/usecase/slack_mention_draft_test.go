...
func TestSlackUseCases_AppMention_CaseBoundChannelDoesNotInvokeDraft(t *testing.T) {
	...
	agent := usecase.NewAgentUseCase(repo, registry, slackMock, nil, nil, nil, llm, llm, agentarchive.NewMemoryHistoryRepository(), agentarchive.NewMemoryTraceRepository(), nil)
