...
3. The top-level text is the title line: a fixed prefix emoji (📌) signals "this row is an Action card", followed by the bold linked title (e.g. `📌 *<webui-url|Investigate the failed login spike>*`). The "Action:" literal was dropped and the per-status emoji was removed from the title — status is communicated via the attachment side-bar color and the Status select element
