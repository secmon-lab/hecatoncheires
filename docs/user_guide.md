...
## Home dashboard (Web UI)

After signing in to the Web UI you land on the **home** screen (`/`), which pulls together, across **every** workspace you have work in, the things that need your attention — rather than dropping you into a single workspace's Case list.

- **Greeting line** — a short, situation-aware one-liner generated by the LLM (a calm nudge, not a stat). It is cached for about an hour and refreshed afterwards, so it stays cheap and varies over time. If no greeting LLM is configured it is simply omitted.
- **My open cases** — every `OPEN` Case you are an assignee of, across all workspaces. A Case that has not been updated for a while shows a **stalled** flag so forgotten work surfaces (the threshold is set with `--dashboard-stale-threshold`).
- **My open actions** — your incomplete Actions on accessible open Cases, ordered by due date with overdue ones first.
- **Choose a workspace** — the workspace picker (the only route into a workspace's pages). **Star** the workspaces you use often; favorites are saved to your account (so they follow you across devices) and float to the top.
...
3. **Planner-driven turn** — the case-draft agent (in
   ...
   **What the thread shows while the turn runs** — the case-draft path
   posts a single `⏳ Drafting a Case from this mention…` placeholder at
   mention time and nothing else until the turn produces a preview or a
   question. It draws no per-round progress lines: the placeholder is
   the whole of the running state.

   The paths that DO draw progress — the thread-mode case agent
   (creation and mentions) and the workspace agent — each keep **one
   message holding one context block**, whose text is replaced at every
   milestone (`🧭 Planning`, `🔎 Investigating (N task(s))`,
   `✅ Round N: N task(s) done`, `🧭 Re-planning`, `📝 Writing the
   answer`). Milestones never accumulate: however long the turn runs,
   it occupies one line in the thread, left showing the last milestone
   it reached. The answer, the case summary and any question form are
   posted as separate messages.
4. **Preview thread reply** — once the planner emits `materialize`, the
   `slackDraftHandler` (host adapter) posts the rendered preview blocks
   as a fresh thread reply and collapses the `⏳ Drafting…` placeholder
   into a short breadcrumb pointing at it. (A workspace switch is the
   exception: there the existing preview is rewritten in place, so the
   switch reads as a same-position change.)
5. **User actions** —
