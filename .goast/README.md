...
3. **`goast eval -p <rule> … --fail ./pkg`** — applies the rules to the whole
   production tree as the CI gate.

Policy files are passed to `goast eval` **individually** (`-p a.rego -p b.rego`),
never as `-p .goast`, so the `*_test.rego` files are not compiled into the
evaluation.
...
4. Run `opa test .goast`, `goast test`, and
   `goast eval -p .goast/<name>.rego ./pkg` (expect 0 real violations, or fix
   the code that violates it).
5. Add the `-p .goast/<name>.rego` line to the `Check code` step in
   `.github/workflows/goast.yml`, and a row to the catalog above.
