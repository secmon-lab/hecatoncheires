...
## Sizing a round is the planner's job, not the runtime's (settled)
- **Each sub-agent's money allowance is set by the planner, per task.** `TaskPlan.BudgetUSD` is a required field once the host wires `Config[T].Remaining`, the parsers reject a round whose budgets are not all positive or whose sum exceeds what the run has left, and `spawnChild` stamps each figure onto that child's Process metadata. Do not add an even-split fallback: the planner is the only party that knows which task it just wrote is the heavy one, and letting it omit the field puts every child back on the run's whole budget.
- **planexec holds no rate and no `ModelPolicy`.** It takes `Config[T].Remaining`, a function the host supplies, and passes it the run's metadata and metrics — the same reason `Finalizers` take the metadata rather than closing over a workspace. A strategy is registered once at startup and then serves every run.
- **The figure is read at the moment of the call**, not carried on the checkpointed state. A round's children can spend a great deal between two of the parent's transitions.
- **`Config[T].Remaining == nil` turns per-task budgets off entirely**, and that is the correct state for a deployment with no LLM configured — `agentkernel.ModelPolicy.RemainingFunc` returns nil for the empty policy, because a policy that prices nothing would report "$0.00 of $0.00" and reject every plan.
- The `## Budget` section of `prompts/planner.md` and `schemaOptions.withBudget` both follow that same flag, so the instruction and the field it asks for cannot appear without each other.
