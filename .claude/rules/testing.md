...
- **For LLM-touching code, use the gollem-provided mocks** (`github.com/gollem-dev/gollem/mock`). Set `NewSessionFunc` / `GenerateFunc` to control the conversation, and inspect the recorded `*Calls()` slices to assert the model was actually invoked with the expected input.
