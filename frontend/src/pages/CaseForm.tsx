...
export default function CaseForm({ caseItem, onClose }: CaseFormProps) {
  ...
  const titleLabel = configData?.fieldConfiguration?.labels?.title as string | undefined
  const descriptionLabel = configData?.fieldConfiguration?.labels?.description as string | undefined
  ...
  return (
    ...
    >
      <div className="col" style={{ gap: 14 }}>
        <div>
          <div className="field-label">{titleLabel ? `${titleLabel} *` : t('labelTitleRequired')}</div>
          <input
            className="input"
            value={title}
        ...
        <div>
          <div className="field-label">{descriptionLabel || t('labelDescription')}</div>
          <textarea
            className="textarea"
            value={description}
